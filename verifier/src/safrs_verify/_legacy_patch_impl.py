...
def _schema_from_example(example: Any) -> dict[str, Any]:
    ...
    if isinstance(example, list):
        item_example = example[0] if example else {}
        return {"type": "array", "items": _schema_from_example(item_example)}
    if isinstance(example, dict):
        properties: dict[str, Any] = {}
        for key, value in example.items():
            if isinstance(key, str):
                properties[key] = _schema_from_example(value)
        return {"type": "object", "properties": properties, "additionalProperties": True}
    return {"type": "string"}


def _normalize_example_backed_schema(schema: dict[str, Any]) -> None:
...
def _operation_response_schemas(operation: dict[str, Any], spec_major: int) -> list[dict[str, Any]]:
...
def _normalize_relationship_data_schema(data_schema: dict[str, Any]) -> dict[str, Any]:
    """
    SAFRS Swagger2 can emit relationship linkage schemas as:
      {"type": "string", "example": {"type": "...", "id": "..."}}
    or with list examples for to-many relationships.
    Normalize these into JSON:API linkage object/array schemas so they can be patched.
    """
    ...
    example = data_schema.get("example")
    if isinstance(example, dict):
        ...
        if isinstance(example_type, str) and example_type:
            normalized["properties"]["type"]["enum"] = [example_type]
            normalized["properties"]["type"]["default"] = example_type
        if example_id is not None:
            normalized["properties"]["id"]["enum"] = [str(example_id)]
            normalized["properties"]["id"]["default"] = str(example_id)
        data_schema.clear()
        data_schema.update(normalized)
        return data_schema
    ...
    if isinstance(example, list) and example:
        ...
        for item in example:
            ...
            item_id = item.get("id")
            if isinstance(item_type, str) and item_type and item_type not in types:
                types.append(item_type)
            if item_id is not None:
                item_id_str = str(item_id)
                if item_id_str and item_id_str not in ids:
                    ids.append(item_id_str)

        item_props = normalized["items"]["properties"]
        if types:
            item_props["type"]["enum"] = types
            item_props["type"]["default"] = types[0]
        if ids:
            item_props["id"]["enum"] = ids
            item_props["id"]["default"] = ids[0]
...
def _relationship_data_schema(body_schema: dict[str, Any]) -> Optional[dict[str, Any]]:
    ...
    data_schema = _normalize_relationship_data_schema(data_schema)
    data_type = data_schema.get("type")
...
def _synthetic_relationship_body_from_seed(
    rel_doc: Any,
    *,
    seed_key: str,
    path: str,
    ...
    if not isinstance(rel_doc, dict) or "data" not in rel_doc:
        raise RuntimeError(
            f"Seed relationship payload for {seed_key} must include 'data' ({method.upper()} {path})"
        )
    ...
    if isinstance(payload_data, list):
        ...
        return {
            ...
            "properties": {
                "data": {
                    "type": "array",
                    "minItems": len(payload_data),
                    "maxItems": len(payload_data),
                    "items": {
                        "type": "object",
                        "properties": {
                            "type": {"type": "string", "enum": types, "default": types[0]},
                            "id": {"type": "string", "enum": ids, "default": ids[0]},
                        },
                    },
                }
            },
    ...
    if isinstance(payload_data, dict):
        item_id = str(payload_data.get("id", ""))
        item_type = str(payload_data.get("type", ""))
        if not item_id or not item_type:
            raise RuntimeError(
                f"Seed relationship payload for {seed_key} must include non-empty id/type ({method.upper()} {path})"
            )
        return {
            ...
            "properties": {
                "data": {
                    "type": "object",
                    "properties": {
                        "type": {"type": "string", "enum": [item_type], "default": item_type},
                        "id": {"type": "string", "enum": [item_id], "default": item_id},
                    },
                }
            },
        }
    ...
    raise RuntimeError(
        f"Seed relationship payload for {seed_key} must use object or array data ({method.upper()} {path})"
    )
    ...
    if data_type == "array":
        ...
        if isinstance(id_schema, dict):
            ...
            if coerced_ids:
                ...
                id_schema["default"] = coerced_ids[0]
...
def _patch_schema_with_seed(schema: dict[str, Any], seed: dict[str, Any]) -> None:
    _normalize_example_backed_schema(schema)
    ...
    for field_name, field_schema in properties.items():
        ...
        if coerced_values:
            ...
            field_schema["default"] = coerced_values[0]
...
def _patch_spec_with_seed(spec: dict[str, Any], seed: dict[str, Any]) -> dict[str, Any]:
    ...
    relationship_path_params_map = seed.get("relationship_path_params")
    for path, operations in paths.items():
        if not isinstance(operations, dict):
            continue
        collection = _collection_from_path(path)
        ...
        for method, operation in operations.items():
            ...
            for response_schema in _operation_response_schemas(operation, major):
                resolved_response_schema = _deref_schema(
                    patched,
                    response_schema,
                    path=path,
                    method=method_lc,
                ...
                _patch_schema_with_seed(resolved_response_schema, seed)

            if method_lc == "post" and path.rstrip("/").endswith("/startswith"):
                responses = operation.setdefault("responses", {})
                if isinstance(responses, dict) and "400" not in responses:
            ...
            if not is_relationship_operation:
                continue
            has_relationship_seed = isinstance(relationships_map, dict) and seed_key in relationships_map
            has_path_param_seed = (
                isinstance(relationship_path_params_map, dict) and seed_key in relationship_path_params_map
            )
            if not has_relationship_seed and not has_path_param_seed:
                # Some generated specs expose relationship operations without seed hints.
                # Skip these operations rather than failing the entire patching pass.
                continue
            ...
            synthetic_relationship_body = _synthetic_relationship_body_from_seed(
                rel_doc,
                seed_key=seed_key,
                path=path,
                method=method_lc,
            ...
            if not isinstance(body_schema, dict):
                resolved_relationship_body = synthetic_relationship_body
            else:
                try:
                    ...
                    body_schema.clear()
                    body_schema.update(synthetic_relationship_body)
                    resolved_relationship_body = body_schema
